...
本規格定義了EPUB 3閱讀系統──處理EPUB出版品的使用者代理，之適用性規定。
...
注意事項
推薦閱讀系統至少支援H.264[h264]與VP8[rfc6386]影片編碼其一，但這並非合規性規定──閱讀系統可以支援任何影片編碼，或者全不支援。閱讀系統開發者在決定支援何種影片格式時，應該考量各項因素，像是是否受到廣泛採用、播放品質以及技術授權費。
3.2 外圍資源
...
除非在本章節中明確定義為覆寫，閱讀系統必須使用定義在[html]中的語意來處理XHTML內容文件，並且尊重任何在之中描述的使用者代理合規性限制。
...
對包含方式，來為嵌入在XHTML內容文件中的SVG添加樣式之目的，閱讀系統必須套用包含文件中可適用的CSS規則到包含的SVG元素中。
...
必須使用定義在[svg]中的語意來處理SVG內容文件，並且尊重任何其中描述的使用者代理合規性限制。除非在本章節中明確定義為覆寫。

必須符合定義在6.4 腳本中的合規性標準。
...
閱讀系統應該在可重排的EPUB內容文件中，支援容器限制腳本[epub-33]。

閱讀系統應該在固定版面文件[epub-33]中支援書脊層級腳本[epub-33]。

閱讀系統應該在使用rendition:flow特性「"scrolled-doc」或「scrolled-continuous」[epub-33]顯示模式的可重排EPUB內容文件中，支援書脊層級腳本。同樣的，如果在可重排EPUB內容文件中支援書脊層級腳本，其必須實作「scrolled-doc」顯示模式，以及應該實作「scrolled-continuous」顯示模式。
...
rendition:page-spread-left以及rendition:page-spread-right特性適用於預分頁及可重排內容，並且僅在閱讀系統創建同步跨頁時套用。

rendition:page-spread-*必須優先於XHTML內容文件中設定page-break-before特性的任何值[csssnapshot]。

當可重排的書脊項目跟在預分頁的項目之後，當其缺少rendition:page-spread-*特性值時，可重排的項目應該由下一頁起──如[epub-33]中page-progression-direction屬性所定義。

閱讀系統必須尊重reflowable與pre-paginated書脊項目中的rendition:page-spread-*特性（例如透過加入空白頁）。

當預分頁的書脊項目跟在可重排的書脊項目之後，當其缺少rendition:page-spread-*特性值時，預分頁的項目應該由下一頁起（如page-progression-direction屬性所定義）。
...
對可重排的內容，支援本特性的閱讀系統必須將每一份虛擬頁面置中。
...
閱讀系統應該使用媒體層疊元素中epub:type屬性所提供的語意資訊，來提供使用者跳過內容的選項。

當啟動跳過內容時，閱讀系統必須停止任何epub:type屬性包含符合可跳過結構的語意 之par以及seq元素的播放。
...
閱讀系統可以支援EPUB內容文件中的結構語意[epub-33]。
...
儘管本規格的主要關注點為如何解析以及處理EPUB出版品，而不強制要求所有的閱讀系統都必須提供特定的使用者介面。但依然有共同的無障礙輔助性問題，所有的閱讀系統開發者都必須注意，或者發現避免在其應用程式中出現。

W3C的使用者代理無障礙指引（User Agent Accessibility Guidelines）[uaag20]提供了許多有用的實踐，開發者應該採用以增進閱讀系統。因為許多瀏覽器會遇到的無障礙輔助性問題，在專為EPUB的使用者代理上都會平行出現。

以下列表概要指出許多一些額外專屬於EPUB的領域，這些領域若缺少無障礙輔助性，會影響到使用者的閱讀體驗：
...
API整合──確保無障礙輔助性樹，包含對role、states以及properties[wai-aria-11]的支援，能夠被作業系統的無障礙輔助性API所取得，如此一來使用者在使用輔助科技時，得以與內容做完整互動。
文件物件模型（Document Object Model, DOM）──提供對EPUB內容文件之[dom]的存取，如此一來使用者可以研究來源中所表述的語意與結構。
目次──確保使用者可以存取連結文字，包含視覺內容的替代文字。應該能夠使用不同的輸入方式觸動目次連結。
DAISY Consortium維護了一份無障礙輔助性測試套件以評量這些以及更多要點。
...
EPUB出版品的特質在於其結構。EPUB格式提供了表現、包裝以及將一組有結構並在語意上強化的網頁內容─包括HTML、CSS、SVG、JavaScript以及其他資源─編碼以單一檔案容器配布的手段。
...
對遠端資源呼叫也可能會被用來追蹤使用者資訊（例如透過伺服器紀錄）。閱讀系統應該限制透過HTTP要求時暴露的資訊，僅能提供取得資源必要的資訊。
...
允許使用者載入不受信任EPUB出版品的閱讀系統（例如未受簽署的EPUB出版品透過「側載（sideloading）」程序）應該將這樣的內容視為不安全（例如，避免使用者允許腳本以及網路存取）。
...
請注意本變更日誌僅列出自EPUB 3.2以來的顯著變更─這些變更會影響閱讀系統的適用性或有同等的重要性。
...
06-10-2022: 將XML外部識別碼的限制從XML適用性敘述移到安全與隱私章節，新增額外的注意事項說明內部或外部實體的安全風險。請見issue 2433 以及issue 2477.
...
28-July-2022: 重構語意關聯機制章節，提供演算法已取得展開的URL。請見issue 2378。
...
17-June-2022: 釐清對語意關聯機制的支援為選擇性。請見issue 2317。
...
27-May-2022: 新增閱讀系統適用性章節。請見issue 2271。
...
27-Aug-2021: 移除推薦對SVG中文字搜尋以及選擇的支援。以更一般性的搜尋無障礙輔助性表列取代，以避免對閱讀系統使用者介面定義規定。請見issue 1774。
...
09-Apr-2021: 新增章節專門說明無障礙輔助性，以及移除對無障礙輔助性規格過時的參照。請見issue 1608。
09-Apr-2021: 新增章節以釐清建立出版品資源主要語言以及基礎行文方向的合規性規定。請見pull request 1613。
01-Apr-2021: 新增章節以釐清如何從包裝文件的相對URL建立絕對URL。請見pull request 1468。
30-Mar-2021: 重構文件，移除重複的適用性章節與規定，其僅是作為定位點用以和製作規定結合。請見pull request 1597以及pull request 1609。
